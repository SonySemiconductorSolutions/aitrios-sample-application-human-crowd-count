...
class CrowdCount(object_detection_processor.ObjectDetectionProcessor):
    ...
    def __init__(self, config):
        ...
        if self._inpolygon_params["area_num"] > self.MAX_AREA_NUM:
            raise RuntimeError("area_num should be less than or equal to ",
                               self.MAX_AREA_NUM)
        ...
        for area_num in range(self._inpolygon_params["area_num"]):
            if (self._inpolygon_params["area_point_len"][area_num]
                    > self.MAX_POINT_NUM):
                raise RuntimeError(
                    "area_point_len should be less than or equal to ",
                    self.MAX_POINT_NUM,
                )
            if self._inpolygon_params["area_point_len"][area_num] < 3:
                raise RuntimeError("area_point_len should be larger than 2 ")
        ...
        if (self._stabilizer_params["iir_down_ratio"] < 0 or
                self._stabilizer_params["iir_down_ratio"] > 1):
            raise RuntimeError(
                "iir_down_ratio must be set in the range of 0 to 1")
        if (self._stabilizer_params["iir_up_ratio"] < 0 or
                self._stabilizer_params["iir_up_ratio"] > 1):
            raise RuntimeError(
                "iir_up_ratio must be set in the range of 0 to 1")
    ...
    def __call__(self, serialize_meta):
        ...
        positions = self._bbox2point(bboxes)
        area_num, count = self._inpolygon(positions)
        count_out = self._stabilizer(area_num, count)
        count_out = count_out[:area_num]

        # Output to dict
        result_dict = self._output_to_dict(bboxes, bboxes_score, positions,
                                            count_out)

        return result_dict

    def _output_to_dict(self, bboxes, bboxes_score, positions, count_out):
        bbox_dicts = []
        for bbox in bboxes:
            bbox_dict = {
                "left": bbox[0],
        ...
        for bbox, score in zip(pre_bboxes, pre_bboxes_score):
            ...
            if min_th <= height <= max_th:
                ...
                if self.DEBUG_CROWD_COUNT:
                    ...
                    print(
                        f"[remove_low_conf]min_th:{min_th:f},max_th:{max_th:f}")
        ...
        for area in range(area_num):
            ...
            for position in positions:
                ...
                for i_1 in range(num_len):
                    i_2 = (i_1 + 1) % num_len
                    if (min(polygon[i_1][0], polygon[i_2][0]) < s_x <= max(
                            polygon[i_1][0], polygon[i_2][0])):
                        if (polygon[i_1][1] +
                            (polygon[i_2][1] - polygon[i_1][1]) /
                            (polygon[i_2][0] - polygon[i_1][0]) *
                            (s_x - polygon[i_1][0]) - s_y) > 0:
                            inside = not inside
