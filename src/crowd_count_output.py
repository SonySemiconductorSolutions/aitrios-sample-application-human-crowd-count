...
class CrowdCountOutput(output.Output):
    ...
    def __init__(self, config, image_info, param_info):
        ...
        if self._output_video is True:
            ...
            video_file_name = os.path.join(
                video_output_dir, f"{output_name}_crowd_count.mp4")
            fourcc = cv2.VideoWriter_fourcc("m", "p", "4", "v")
            self._video_writer = cv2.VideoWriter(video_file_name, fourcc,
                                                 frame_rate,
    ...
    def __call__(self, dict_meta, image=None, timestamp=None):
        ...
        with open(
                os.path.join(self._json_output_dir, f"{json_file_name}.json"),
                "w",
                encoding="utf-8",
        ) as file:
        ...
        if self._output_video is True and image is not None:
            ...
            for polygon, num_len in zip(
                    self._param_info["area_point"],
                    self._param_info["area_point_len"],
            ):
            ...
            for bbox, score in zip(dict_meta["bboxes"],
                                   dict_meta["bboxes_score"]):
                cv2.rectangle(
                    image,
                    (bbox["left"], bbox["top"]),
            ...
            for position in dict_meta["positions"]:
                cv2.drawMarker(
                    image,
            ...
            for area in range(self._param_info["area_num"]):
                text = f'Count(area{str(area + 1)}):{str(dict_meta["count"][area])}'
                (text_width, text_height), baseline = cv2.getTextSize(
                    text, cv2.FONT_HERSHEY_SIMPLEX, font_scale, 0)
                cv2.putText(
                    image,
                    text,
                    (5, ((2 * text_height) * (area + 1))),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    font_scale,
                    (0, 225, 0),
                    1,
            ...
            if timestamp is not None:
                timestamp_text = f'{timestamp[:4]}-{timestamp[4:6]}-{timestamp[6:8]} {timestamp[8:10]}:{timestamp[10:12]}:{timestamp[12:14]}'

                cv2.putText(
                    image,
                    timestamp_text,
                    (5, self._height - (2 * text_height)),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    font_scale,
                    (0, 225, 0),
                    1,
